...
// Get all issues
router.get('/', (req, res) => {
    const issues = query('SELECT * FROM issues ORDER BY createdAt DESC');
    res.json(issues);
});
...
// Create issue
router.post('/', (req, res) => {
    const { title, description, location, impact, status } = req.body;
    const { lastId } = run('INSERT INTO issues (title, description, location, impact, status) VALUES (?, ?, ?, ?, ?)', [title, description || '', location || '', impact || '', status || 'open']);
    const issue = queryOne('SELECT * FROM issues WHERE id = ?', [lastId]);
